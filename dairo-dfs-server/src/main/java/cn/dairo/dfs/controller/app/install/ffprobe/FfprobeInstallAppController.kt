package cn.dairo.dfs.controller.app.install.ffprobe

import cn.dairo.dfs.config.Constant
import cn.dairo.dfs.controller.app.install.ffprobe.form.FfprobeInstallProgressForm
 ...
 */
@Controller
@RequestMapping("/app/install/ffprobe")
class FfprobeInstallAppController : AppBase() {
    ...
    @ResponseBody
    fun install() {
        if (File(Constant.FFPROBE_PATH).exists()) {
            return
        }
        if (this.isRuning) {
            return
    ...
    @ResponseBody
    fun progress(): FfprobeInstallProgressForm {
        val form = FfprobeInstallProgressForm()
        form.info = this.info
...
//        form.url = FFPROBEInstallAppController.DOWNLOAD_URL
